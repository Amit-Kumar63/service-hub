...
useGSAP(()=> {
  ...
  const bookings = [
    { id: "B123", serviceName: "Plumbing Fix", user: "John Doe", date: "2025-01-15", status: "Pending" },
    { id: "B124", serviceName: "Electrical Setup", user: "Jane Smith", date: "2025-01-17", status: "Completed" },
    { id: "B126", serviceName: "Plumbing Fix", user: "John Doe", date: "2025-01-15", status: "Pending" },
    { id: "B128", serviceName: "Electrical Setup", user: "Jane Smith", date: "2025-01-17", status: "Completed" },
  ];
  return (
    <div className="p-4 space-y-6 bg-gray-100 min-h-screen relative">
      ...
      <div className="space-y-4">
        ...
        <h1 className="text-xl font-bold self-end text-gray-800">Dashboard</h1>
...
<div className="space-y-4">
        ...
        <button className="w-full bg-blue-500 text-white py-2 rounded" onClick={() => {setAddServicePanel(!addServicePanel); addServicePanel ? setValue(0) : setValue(1)}}>
          Add New Service
        </button>
      </div>
      ...
      <div className="space-y-4 pb-16">
        ...
        {bookings.map((booking) => (
          <div key={booking.id} className="bg-white p-4 rounded shadow">
            ...
            <p className="text-gray-500">Booked by: {booking.user}</p>
            <p className="text-gray-500">Date: {booking.date}</p>
            <p className={`text-sm ${booking.status === "Pending" ? "text-yellow-500" : "text-green-500"}`}>
              {booking.status}
            </p>
          </div>
        ))}
      </div>
      <div ref={addServicePanelRef} className="fixed translate-y-full h-fit left-0 right-0 bottom-0 z-10 w-full bg-white"> 
        <AddService setAddServicePanel={setAddServicePanel} setValue={setValue}/>
      </div>
      <div className="fixed bottom-0 z-10 right-0 left-0 border border-t border-gray-300">
      <ProviderNavigation value={value} setValue={setValue} setAddServicePanel={setAddServicePanel} addServicePanel={addServicePanel}/>  
      </div>
    </div>
  );
};
